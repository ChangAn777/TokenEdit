...
class TokenEditUtils:
    ...
    def find_subject_positions(
        ...
        修复:
        - 统一tokenization策略(add_special_tokens参数)
        - 更鲁棒的模糊匹配
        - 处理multi-token subjects
        
        Args:
            prompt: 输入文本 "The capital of France is"
            subject: 主体 "France"
            verbose: 是否打印调试信息
            add_special_tokens: 是否添加特殊token(必须与inference时一致)
        
        Returns:
            positions: 主体token的位置列表 [3, 4] (如果"France"被分成2个token)
        """
        # 修复: 统一使用相同的tokenization策略
        ...
        if not positions:
            ...
            if text_start != -1:
                ...
                before_ids = self.tokenizer.encode(before_text, add_special_tokens=add_special_tokens)
    ...
    def compute_kl_divergence(
        ...
        # 修复: 正确的KL散度计算
        # KL(P_edit || P_orig) = sum(P_edit * (log P_edit - log P_orig))
        kl_loss = F.kl_div(
            orig_log_probs,  # log(P_orig)
            edit_log_probs,  # log(P_edit)
            log_target=True,
            reduction='batchmean'
        )
